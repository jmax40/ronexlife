...
<?php
include_once '../process/foreach_transaction_productperformance_chart .php';
...
<div id="donutchart2" style="height: 400px; width: 200px; background-color: lightblue; padding: 5px;"></div>
...
<table>
    ...
    <tbody>
        <?php foreach ($coordinatorTotals as $coordinator => $totalAmount): ?>
            <tr>
                <td><?php echo htmlspecialchars($coordinator); ?></td>
                <td><?php echo htmlspecialchars($totalAmount); ?></td>
            </tr>
        <?php endforeach; ?>
        <td colspan="4">
            <button style="width: 100%; height: 100%; padding: 10px; background-color: #4CAF50; color: white; border: none; cursor: pointer;">
               Veiw Details
            </button>
        </td>
    </tr>
...
<script>
// Get the product totals data from PHP
var productTotals = <?php echo $jsonProductTotals; ?>;
...
for (var product in productTotals) {
    if (productTotals.hasOwnProperty(product)) {
        morrisData.push({
            product: product,
            total: productTotals[product] // Total amount for each product
        });
    }
}
...
new Morris.Bar({
    ...
    xkey: 'product', // The key for the x-axis (product names)
    ykeys: ['total'], // The key for the y-axis (total amounts)
    labels: ['Total Amount'], // Label for the ykeys
...
new Morris.Donut({
    element: 'donutchart2', // The ID of the element in which to draw the chart
    data: [
        { label: 'Agent 1', value: 50 },
        { label: 'Agent 2', value: 25 },
        { label: 'Agent 3', value: 15 },
        { label: 'Agent 4', value: 10 },
        { label: 'Agent 5', value: 50 },
        { label: 'Agent 6', value: 25 },
        { label: 'Agent 7', value: 15 },
        { label: 'Agent 8', value: 10 },
        { label: 'Agent 9', value: 50 },
        { label: 'Agent 10', value: 10 }
    ],
    colors: ['#0b62a4', '#7A92A3', '#4da74d', '#afd8f8'], // Colors for each section
    formatter: function (y) { return y + "%" } // Custom label formatting
});
